...
class CGCNN(torch.nn.Module):
        ...
        **kwargs
        ...
        if pre_fc_count > 0:
            ...
            for i in range(pre_fc_count):
                if i == 0:
                    lin = BNNLayer(data.num_features, dim1, activation='none', prior_mean=0, prior_rho=0)
                    self.pre_lin_list.append(lin)
                else:
                    lin = BNNLayer(dim1, dim1, activation='none', prior_mean=0, prior_rho=0)
                    self.pre_lin_list.append(lin)
        elif pre_fc_count == 0:
            self.pre_lin_list = torch.nn.ModuleList()
    ...
    def forward(self, data, mode = 'forward'):
        
        tkl = 0
        ##Pre-GNN dense layers
        for i in range(0, len(self.pre_lin_list)):
            if i == 0:
                out,kl = self.pre_lin_list[i](data.x)
                tkl += kl
                out = getattr(F, self.act)(out)
                #prev_out = out
            else:
                out,kl = self.pre_lin_list[i](out)
                tkl += kl
                out = getattr(F, self.act)(out)
        ...
        if self.pool_order == "early":
            ...
            for i in range(0, len(self.post_lin_list)):
                out, kl = self.post_lin_list[i](out)
                tkl += kl
                out = getattr(F, self.act)(out)
                #out = torch.add(out, prev_out)
                #prev_out = out
